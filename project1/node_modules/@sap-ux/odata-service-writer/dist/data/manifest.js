...
function enhanceManifestDatasources(fs, webappPath, dataSourceUpdateSettings) {
    ...
    const createdLocalAnnotationDataSources = addLocalAnnotationDataSources(dataSources, serviceName, serviceLocalAnnotations, forceServiceUpdate);
    const settings = {
 ...
 */
function addLocalAnnotationDataSources(dataSources, serviceName, serviceLocalAnnotations, useOldAnnotations) {
 ...
 */
function removeUnusedAnnotations(fs, webappPath, dataSources, serviceName, createdAnnotations) {
    ...
    serviceAnnotations.forEach((name) => {
        ...
        if (dataSource?.type === 'ODataAnnotation' &&
            ...
            if (oldAnnotationPath && fs.exists((0, path_1.join)(webappPath, oldAnnotationPath))) {
                fs.delete((0, path_1.join)(webappPath, oldAnnotationPath));
            }
 ...
 * @returns created annotation dataSources list for service.
 */
function addRemoteAnnotationDataSources(fs, webappPath, dataSources, serviceName, serviceRemoteAnnotations, cleanOldAnnotations) {
    const createdAnnotations = [];
 ...
 * @param {Editor} fs - the memfs editor instance
 */
function convertSingleService(webappPath, dataSourceKey, dataSource, fs) {
 ...
 */
async function addMultipleServiceSupportToManifest(webappPath, manifest, fs) {
    const dataSources = manifest?.['sap.app']?.dataSources;
